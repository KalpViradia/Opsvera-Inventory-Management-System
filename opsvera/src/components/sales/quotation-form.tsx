...
import { createQuotationSchema } from "@/validations/sales";
...
export function QuotationForm({ customers, products }: QuotationFormProps) {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<z.infer<typeof createQuotationSchema>>({
    // @ts-expect-error - Zod resolver types mismatch with the defaultValues, but it works fine at runtime
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
      ...
      {/* Currency Settings */}
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-4 rounded-lg border border-dashed bg-muted/20">
      ...
      <div className="space-y-2">
        ...
        <Textarea 
          id="notes" 
          placeholder="Terms, conditions, validity details, etc." 
          {...register("notes")} 
          rows={3}
        />
      </div>
