...
import { createPOSchema } from "@/validations/purchase";
...
export function POForm({ suppliers, products }: POFormProps) {
  ...
  const {
    ...
    watch,
    formState: { errors },
  } = useForm<z.infer<typeof createPOSchema>>({
  ...
  const totalTax = watchItems.reduce((acc, item) => acc + (item.quantity * item.unitPrice * (item.taxRate / 100)), 0);
  const grandTotal = subtotal + totalTax;

  const onSubmit = async (data: z.infer<typeof createPOSchema>) => {
    setIsLoading(true);
    try {
        ...
        <div className="space-y-2">
          ...
          >
            ...
            {suppliers.map((supplier) => (
              <option key={supplier.id} value={supplier.id} className="bg-background text-foreground">
                {supplier.name}
              </option>
            ))}
          </select>
